...
const Paginator = ({
  ...
  if (numPages < 8) {
    return (
      <Pagination
        aria-label="Sidenavigering i tabell"
        className={'pagination justify-content-end ' + styles.paginator}
      >
  ...
  return (
    ...
    >
      ...
      {pageIndex > 2 &&
        ...
        [pageIndex - 1, pageIndex, pageIndex + 1].map((page) =>
          toPageTab(page)
        )}
      {pageIndex > lastPage - 3 ? (
        [lastPage - 3, lastPage - 2, lastPage - 1].map((page) =>
          toPageTab(page)
        )
      ) : (
